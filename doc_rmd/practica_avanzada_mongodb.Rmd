...
Vamos a utilizar datos que el ayuntamiento de Madrid tiene en su portal de datos abiertos. En particular, vamos a trabajar con los datos en este [enlace](https://opendata.emtmadrid.es/Datos-estaticos/Datos-generales-(1)) que se corresponden con el uso del servicio de bicicletas eléctricas de ayuntamiento de Madrid: **BiciMAD**.

En la web aparecen dos conjuntos de datos, los que se llaman `Datos de uso de ...` contienen la traza de todos los movimientos que se han producido. Es decir, información de cada vez que un usuario ha cogido una bici en una estación y la ha dejado en otra.

Los que se denominan `Situación estaciones bicimad por día y hora de ...` recogen información de cada estación en diversos instantes temporales.
...
Esto habrá lanzado el *motor* de MongoDB. Si queremos entrar en la *shell* de MongoDB, abrimos una nueva ventana de la terminal y escribimos `mongo`.
...
Después, vamos a intentar entender la estructura de los documentos. Si ejecutamos `db.usage.findOne()` obtendremos el primer documento para una inspección rápida.
...
Una cosa que podemos ver en los tres primeros documentos es el **código postal**. Para extraer solamente el código postal de las colecciones, podemos hacer con `db.usage.find({}, {zip_code: 1})` de donde obtendremos:
...
En muchos documentos, el campo `zip_code` sin informar. Como cada documento puede tener una estructura distinta, vamos a hacer que desaparezca este campos en aquellos documentos en los que esté sin informar.
...
```{}
b.usage.find({zip_code: {$exists: 0}}).pretty()
```


## Agregación

Una operación habitual en cualquier base de datos es la de **agregar** datos. Los operadores de agregación agrupan valores de múltiples documentos juntos y pueden realizar opreaciones para devolver un único resultado. Tenemos el *aggregation pipeline* para hacer agregaciones

![](https://docs.mongodb.com/manual/_images/aggregation-pipeline.bakedsvg.svg)

El *aggregation pipeline* de MongoDB consiste en *stages*. Cada *stage* transforma los documentos como si pasasen por el *pipeline*. El *stage pipeline* no es necesartio que produzca un *output* para cada documento de *input*.

Vamos a empezar calculando **cuántos viajes de producen para cada estación de origen (campo `ipunplug_base`)**. En código SQL, haríamos 
...
El campo `_id` es **obligatorio**. Para referirnos a un campo del documento, haremos "$idunplug_base". Para hacer el conteo, deberemos recurrir al operador suma `$sum$. La sentencia sería
...
Por ejemplo, si nuestor interés principal es el de analizar el tiempo de viaje en función de las estaciones de partida, sería interesante tener una estructura como
...
> **Ejercicio:** utilizando la última sentencia, haz que el resultado esté ordenado primero por día del mes y luego por mes.
...
> **Ejercicio:** Calcula le media de las bases libres para cada estación usando el stage `$unwind`.
...
En las bases de datos relacionales existía el concepto de vista. Las vistas son consultas almacenadas (solo la información de cómo hacer la consulta, no los datos en sí mismo) que, de cara al usuario, actuan como si fuese una tabla física. En MongoDB podemos crear vistas de colecciones con [`createView()`](https://docs.mongodb.com/manual/reference/method/db.createView/) cuya sintaxis es

```{}
db.createView(<view>, <source>, <pipeline>, <options>)
```

Donde `<view>` es el nombre que le queremos dar a la vista entre comillas, `<soruce>` el nombre de la colección de la que queremos crear una vista y `<pipeline>` un *array* tal y como lo usaríamos en un *aggregation pipeline*.
...
Vamos a hacer un ejemplo entre todos. Primero, vamos a crear un vista con el maestro de las estaciones. **Escribe** una sentencia que devuelva
...
Y genera una vista que se llame `info_estaciones`.

Ahora vamos a añadir la dirección de la estación de origen para cada documento de la colección `usage` a través de el stage [`$lookup`](https://docs.mongodb.com/manual/reference/operator/aggregation/lookup/)
